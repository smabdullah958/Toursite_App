  ...
  return (
    <div>
      ...
      <button
        ...
        className="text-sm font-semibold text-amber-600 hover:text-amber-700 hover:underline transition duration-300 2xl:text-4xl p-3">
        Forgot Password?
      </button>
      ...
      {DisplayForgetPassword && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-gradient-to-br from-amber-50 to-yellow-50 p-8 rounded-2xl shadow-2xl w-[420px] max-w-full flex flex-col animate-fadeIn border border-amber-200  2xl:w-[20vw] 2xl:h-[20vh]">
            <h2 className="text-2xl font-bold text-center text-amber-900 mb-6 2xl:text-4xl py-3">
              Reset Password
            </h2>

            <div className="mb-4">
              <p className="  text-red-500 text-sm 2xl:text-2xl">{errors.Email?.message}</p>
              <span className={` text-sm 2xl:text-2xl ${(errorMessage==="User is not found")?"text-red-500":"text-green-600"}`}>{errorMessage}</span>

              <input
                type="email"
                placeholder="Enter your email"
                {...register("Email")}
                className="border w-full px-4 py-2 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition-all border-amber-200 bg-white 2xl:px-6 2xl:py-4 mt-5"
              />
