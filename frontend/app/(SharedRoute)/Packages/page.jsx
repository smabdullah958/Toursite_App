...
const PackagesPage = () => {
  ...
  }, [dispatch, result.length, IsLogIn, Role]);
  ...
  const filteredResult = displayResult.filter((pkg) => {
    if (Role === "Admin") return true;
return pkg.BookingOption?.some(option=>option.Slots > 0)
  });
  ...
  const getAvailablePrice = (pkg) => {
    // If no booking options exist, return N/A
    if (!pkg.BookingOption || pkg.BookingOption.length === 0) return "N/A";
    
    // Find first booking option with slots > 0
    const availableOption = pkg.BookingOption.find(opt => opt.Slots > 0);
    ...
    if (Role === "Admin" && pkg.BookingOption[0]) {
      return pkg.BookingOption[0].BasePrice;
    }
  ...
  return (
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-12">
        <div className="text-center mb-8">
          <h1 className="text-2xl sm:text-5xl font-extrabold text-gray-900 mb-4">
            Explore Our <span className="bg-gradient-to-r from-amber-800 via-yellow-700 to-amber-600 bg-clip-text text-transparent">Premium Packages</span>
          </h1>
    ...
    blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8/+F9PQAI8wNP6F3jYwAAAABJRU5ErkJggg=="         
        ...
        <span className="text-lg font-bold">AED  {" "+getAvailablePrice(pkg)}</span>
                </div>
